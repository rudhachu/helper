...
This repository shows how to send every currently known WhatsApp interactive / native flow button type using WhiskeySockets (Baileys fork) without modifying core source. The functionality is packaged and published as the npm package `rudhra-btn` which reproduces the binary node structure the official client emits so buttons render correctly for both private & group chats.

## Problem Statement

By default, WhiskeySockets cannot send interactive buttons while itsukichan can. The root cause is that WhiskeySockets lacks the required binary node wrappers (`rudhra`, `interactive`, `native_flow`) that WhatsApp expects for interactive messages.

## Solution

The enhanced functionality provided by the `rudhra-btn` package provides the missing functionality by:
...
3. **Adding missing binary nodes** (`rudhra`, `interactive`, `native_flow`, `bot`) via `additionalNodes`
...
- ✅ **Private chat support** (adds `bot` node with `rudhra_bot: '1'`)
- ✅ **Group chat support** (adds only `rudhra` node)
- ✅ **Backward compatibility** (regular messages pass through unchanged)

## Quick Start (Most Common Case)

```javascript
const { sendButtons } = require('rudhra-btn');
...
```javascript
const { sendInteractiveMessage } = require('rudhra-btn');
...
Private chat: adds `rudhra` + `interactive/native_flow` + `bot (rudhra_bot=1)`.

Group chat: adds only `rudhra` + `interactive/native_flow`.
...
5. Injects required binary nodes:
   - Always a `rudhra` node (with nested `interactive/native_flow/...` for buttons and lists) when interactive.
   - Adds `{ tag: 'bot', attrs: { rudhra_bot: '1' } }` automatically for private (1:1) chats enabling rendering of interactive flows.
...
await sendInteractiveMessage(sock, jid, {
  ...
  additionalNodes: [ { tag: 'rudhra', attrs: { experimental_flag: '1' } } ] // will be merged before auto interactive nodes
});
```
...
|-------------------|-----------------------|-------|
| `review_and_pay`  | `rudhra` with `native_flow_name=order_details` | Payment/order style flow |
| `payment_info`    | `rudhra` with `native_flow_name=payment_info`  | Payment info flow |
| `mpm`, `cta_catalog`, `send_location`, `call_permission_request`, `wa_payment_transaction_details`, `automated_greeting_message_view_catalog` | `rudhra > interactive(native_flow v=1) > native_flow(v=2,name=<name>)` | Specialized (may require official client) |
| Anything else / mixed | `rudhra > interactive(native_flow v=1) > native_flow(v=9,name=mixed)` | Generic path covering standard quick replies, lists, CTAs |
...
- If buttons do not render: ensure first binary node injected is `rudhra` and private chats include the `bot` node.
...
- `lib/buttons.js` - Enhanced with binary node support (template functionality removed)
- `main.js` - Central main surface for the package and metadata helper
